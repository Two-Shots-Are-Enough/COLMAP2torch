...
class DatasetRE10k(IterableDataset):
    ...
    def __iter__(self):
        ...
        for chunk_path in self.chunks:
            ...
            for run_idx in range(int(times_per_scene * len(chunk))):
                ...
                # 오히려 여기서는 re10k는 det가 1이 안나오고 내껀 1이 나온다. 근데 아래에 convert_poses 안에서는 오히려 내께 value error가 뜬다.
                # 그니까 지금 format_changer에서 애초에 이상하단 거지. 여기선 오히려 det 1이 안 나와야 맞고 밑에서 1이 나와야 맞다.
                ...
                num_context_images = len(context_indices)
                num_target_images = len(target_indices)
                print(f"Test sample - Number of context images: {num_context_images}, Number of target images: {num_target_images}")
    ...
    def convert_poses(
        ...
        # Verify that each rotation matrix has a determinant close to 1
        #print(w2c[:, :3, :3])
        det_w2c_rotations = torch.det(w2c[:, :3, :3])
        print(det_w2c_rotations)
        if not torch.allclose(det_w2c_rotations, torch.ones_like(det_w2c_rotations), atol=1e-3):
            raise ValueError("src/dataset/dataset_re10k.py: Detected w2c rotation matrices with determinant not close to 1 before inversion.")
